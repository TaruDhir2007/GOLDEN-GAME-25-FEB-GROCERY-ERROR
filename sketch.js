...
setTimeout(() => {  
  ...
  perfectSound.stop();
    ...
    if (laughScore === 1 || speechScore === 1
       || wordScore === 1 || replyScore === 1 || laughScore === 0 || speechScore === 0 || wordScore === 0 || replyScore === 0){
      badP.visible = true;
      badP.depth = presentation.depth;
      badP.depth += 1;
      count=7;
      setTimeout(() => {  badP.visible = false; }, 5500);
    } 
    
    if (laughScore === 1 && speechScore === 1 || wordScore === 1 && replyScore === 1 ||laughScore === 1 && replyScore === 1
       || wordScore === 1 && laughScore === 1 || speechScore === 1 && wordScore === 1 || speechScore === 1 && replyScore === 1
       || laughScore === 1 && speechScore === 1 && wordScore === 1 || laughScore === 1 && speechScore === 1 && replyScore === 1 ||
       laughScore === 1 && replyScore === 1 && wordScore === 1 || replyScore === 1 && speechScore === 1 && wordScore === 1){
      fineP.visible = true;
      fineP.depth = presentation.depth;
      fineP.depth += 1;
      count=7;
      setTimeout(() => {  fine.visible = false; }, 5500);
    }
    if (laughScore === 1 && speechScore === 1 && wordScore === 1 && replyScore === 1){
      goodP.visible = true;
      goodP.depth = presentation.depth;
      goodP.depth += 1;
       count=7;
       setTimeout(() => {  goodP.visible = false; }, 5500);
    }}
  }
  }
function live_5(){
...
if(mousePressedOver(butter));{
butter.destroy();
grocerySound.loop=false;
  
if(! grocerySound.isPlaying()){
  grocerySound.play();
}}
